...
<head>
    ...
    <div class="loadding">
        ...
        <div class="spinner">
            <img src="${staticFileContextPath}/static/images/wap/cqssc/LOGO.jpg" alt=""/>
    ...
    <script src="${staticFileContextPath}/static/js/wap/cqssc/phoenix.hack.js"></script>
    <script src="${currentContextPath}/gameBet/cqssc/config"></script>
	<script src="${staticFileContextPath}/static/js/wap/cqssc/jquery-2.1.4.min.js"></script>
    <script type="text/javascript">
        window.debug = false;
        var returnUrl = '${userContextPath}';
        var basePath = '';
        var introUrl = '${userContextPath}/help/wap/queryLotteryDetail?helpId=869';
        var moreRecordUrl = '/game/chart/cqssc/Wuxing';
        var backToPcUrl = '/gameBet/cqssc?isPass=true';
        ...
        // 调用接口获取近期开奖信息
        var gameChartUrl = '/game/chart/cqssc';
    ...
    <link href="${staticFileContextPath}/static/images/wap/cqssc/app.min.css" rel="stylesheet">
</head>
        ...
        <div class="bar bar-header bar-stable">
            ...
            <div class="ar-popmenu {{menuShow? '':'menuShow'}}">
                <ul>
                    ...
                    <li ng-click="yuanjiao()">模式 {{ moneyUnit==1?"元":"角" }}</li>
                    <li ng-click="fandian()" style="display: {{serverGameConfig.awardGroupRetStatus==1?'':'none'}}">奖金返点</li>
            ...
            <div class="select-result">
                <dl class="result-money confirm-money">
                    <!--<dt>共{{ totalObject.totalCount }}注，<span>{{(totalMoney = totalObject.totalM * globelMultipleData.multiple| number:1)  }}元 </span></dt>-->
					 <dt>{{ totalObject.totalCount }}注x{{globelMultipleData.continuesBet}}期x{{globelMultipleData.multiple}}倍=<span>{{(totalMoney = totalObject.totalM * globelMultipleData.multiple*globelMultipleData.continuesBet| number:1)}}元 </span></dt>
...
<script id="templates/pick.html" type="text/ng-template">
    <ion-view ng-click="cloaseAllWindow()" xmlns:btsp="http://www.w3.org/1999/html">
        ...
        <div class="ar-poptype" onscroll="true" ng-class="{active: poptypeshow}" style="overflow: visible; position: absolute; z-index: 3;">
            <div style="width:100%;height: 0px; background:rgba(0,0,0,0.6); position: absolute; top:315px;" id="methodBox"></div>
            <!--业务模板数据-->
                <div class="strategy-select">
                    ...
                    <div style="width:100%; overflow: hidden;border-bottom: 1px solid #e6e6e6; position: relative;">
                        <div class="row" id="play_type_box">
        ...
        <div class="game-history {{isDanshi? 'isDanshi':'' }}" style="top:45px;">
            <div style="width:100%; position:relative;background: rgba(0,0,0,0.6);height: 0px;" id="historyBox">
                    ...
                    <div class="history-box {{historyShow ? '': 'record-list'}}">
                        <table class="table table-hover morespecial" cellpadding="0" cellspacing="0">
                            ...
                            <tr ng-repeat="item in historyRecord">
                                ...
                                <td>{{item.kuadu}}</td>
                                <td>{{item.totValue}}</td>
                                <td><spen class="icon {{item.super2000?'ion-android-done':'ion-android-close'}}"></spen></td>
                            </tr>
                            </tbody>
                        </table>
        ...
        <ion-content  class="has-footer has-header " delegate-handle="mainScroll" style="top:77px;">
            <!--选号容器-->
            <div class="number-select-content {{isDanshi? 'isDanshi':'' }}">
                ...
                <div class="number-select-list" ng-repeat="(ballLabel,balls) in ballsTree">
                    ...
                    <ul class="ball-list" style="clear:both;">
        ...
        <div class="bar bar-footer bar-stable {{ hasChoose && !isDanshi ?'active':'' }}">
            ...
            <div ng-hide="iseditor">
                <div class="select-result">
                    ...
                    <dd>{{ shopCar.count * shopCar.onePrice * shopCar.moneyUnit}}元</dd>
                        <!--业务模板数据-->
                    </dl>
...
<script id="templates/submit.html" type="text/ng-template">
    <ion-view view-title="投注成功">
            ...
            <div class="result-box">
                <dl class="result-bet-info">
                    ...
                    <!--<dd>{{(totalMoney = totalObject.totalM * globelMultipleData.multiple| number:1)  }}元</dd> modify by feiker 2016-8-18 14:16:02-->
                     <dd>{{(totalMoney = totalObject.totalM * globelMultipleData.multiple * globelMultipleData.continuesBet| number:1)  }}元</dd>   
                    <dt>投注内容</dt>
                    <dd></dd>
                </dl>
                <div class="result-list">
        ...
        <div class="bar bar-footer bar-foot-submit-finished">
            <button class="button button-full button-submit-finished" ng-click="goPick()">继 续 投 注</button>
        </div>
...
<script src="${staticFileContextPath}/static/js/wap/cqssc/game.js"></script>
<script src="${staticFileContextPath}/static/js/wap/cqssc/app.min.js"></script>

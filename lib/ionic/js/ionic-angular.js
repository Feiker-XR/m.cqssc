...
/**
 * @ngdoc method
 * @name $ionicConfigProvider#templates.bak.maxPrefetch
 * @description Sets the maximum number of templates.bak to prefetch from the templateUrls defined in
 ...
 * @usage
 * State templates.bak are cached automatically, but you can optionally cache other templates.bak.
 *
 * ```js
 ...
 *     $ionicConfigProvider.templates.bak.prefetch(false);
 *
 ...
 *         templateUrl: "tabs-templates.bak/tabs.html"
 *       })
 *       .state('tabs.home', {
 *         url: "/home",
 *         views: {
 *           'home-tab': {
 *             prefetchTemplate: false,
 *             templateUrl: "tabs-templates.bak/home.html",
...
function($http, $templateCache, $timeout) {
  ...
  function run() {
    ...
    while (i < 4 && (template = toCache.pop())) {
      // note that inline templates.bak are ignored by this request
      if (isString(template)) $http.get(template, { cache: $templateCache });
      i++;
    }
    // only preload 3 templates.bak a second
    if (toCache.length) {
      $timeout(run, 1000);
    }
  }
 ...
 *
 * The ionNavView directive is used to render templates.bak in your application. Each template
 ...
 *
 * Pages are loaded by the URLs given. One simple way to create templates.bak in Angular is to put
